...
public sealed class VectorSurface : VisualElement {
 ...
 public void Text(float x,float y,string text,float wrap){if(context==null||string.IsNullOrEmpty(text))return;if(wrap>0){string line="";foreach(var rune in Tokens(text)){
 if(rune=="\n"){Text(x,y,line,0);y+=state.size*state.lineHeight;line="";continue;}
 if(line.Length>0&&Measure(line+rune)>wrap){int space=line.LastIndexOf(' ');
  if(space>0){Text(x,y,line.Substring(0,space),0);line=line.Substring(space+1)+rune;}
  else {Text(x,y,line,0);line=rune==" "?"":rune;}
  y+=state.size*state.lineHeight;
 }else line+=rune;
 }if(line.Length>0)Text(x,y,line,0);return;}
