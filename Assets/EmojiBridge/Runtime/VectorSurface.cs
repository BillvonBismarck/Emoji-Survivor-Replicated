...
public sealed class VectorSurface : VisualElement {
 ...
 public FontAsset MainFont,PixelFont,EmojiFont;
 public Dictionary<string,string> EmojiSequences=new Dictionary<string,string>();
 IEnumerable<string> Tokens(string text){text=text.Replace("\uFE0F","");for(int i=0;i<text.Length;){string token=null;int consumed=0;for(int n=Math.Min(24,text.Length-i);n>1;n--){if(EmojiSequences.TryGetValue(text.Substring(i,n),out token)){consumed=n;break;}}if(consumed==0){consumed=char.IsHighSurrogate(text[i])&&i+1<text.Length?2:1;token=text.Substring(i,consumed);}i+=consumed;yield return token;}}
 ...
 void Draw(MeshGenerationContext c){context=c;geometry.Clear();state=new State{matrix=Matrix4x4.identity,fill=Color.white,stroke=Color.white,alpha=1,width=1,size=20,lineHeight=1.2f,align=1,font=2,clip=new Rect(0,0,720,1280)};stack.Clear();paths.Clear();try{Render?.Invoke();Flush();}catch(Exception e){Debug.LogError(e);}context=null;}
 ...
 void Stroke(List<Vector2> ps){var color=state.stroke;color.a*=state.alpha;float half=state.width*Scale*.5f;for(int i=1;i<ps.Count;i++){var a=ps[i-1];var b=ps[i];var delta=b-a;var n=new Vector2(-delta.y,delta.x).normalized*half;Triangle(a-n,a+n,b+n,color,color,color);Triangle(a-n,b+n,b-n,color,color,color);}if(state.cap==1)foreach(var p in ps){for(int i=0;i<10;i++){float a=i*Mathf.PI/5,b=(i+1)*Mathf.PI/5;Triangle(p,p+new Vector2(Mathf.Cos(a),Mathf.Sin(a))*half,p+new Vector2(Mathf.Cos(b),Mathf.Sin(b))*half,color,color,color);}}}
 ...
 void Triangle(Vector2 a,Vector2 b,Vector2 c,Color ca,Color cb,Color cc){if(context==null)return;clipA.Clear();clipA.Add(new CV(a,ca));clipA.Add(new CV(b,cb));clipA.Add(new CV(c,cc));var src=clipA;var dst=clipB;for(int edge=0;edge<4;edge++){dst.Clear();if(src.Count==0)return;var prev=src[src.Count-1];float pd=Distance(prev.p,edge);foreach(var cur in src){float cd=Distance(cur.p,edge);if((pd>=0)!=(cd>=0)){float t=pd/(pd-cd);dst.Add(new CV(Vector2.Lerp(prev.p,cur.p,t),Color.Lerp(prev.c,cur.c,t)));}if(cd>=0)dst.Add(cur);prev=cur;pd=cd;}var tmp=src;src=dst;dst=tmp;}
 ...
 FontAsset CharacterFont(string s){uint ch=(uint)char.ConvertToUtf32(s,0);if(ch>=0x2300&&EmojiFont!=null){EmojiFont.TryAddCharacters(s,out _);if(EmojiFont.characterLookupTable.ContainsKey(ch))return EmojiFont;}Font.TryAddCharacters(s,out _);if(Font.characterLookupTable.ContainsKey(ch))return Font;MainFont.TryAddCharacters(s,out _);return MainFont;}
 float Advance(string s,FontAsset font){if(s=="\uFE0F"||s=="\uFE0E"||s=="\u200D")return 0;uint ch=(uint)char.ConvertToUtf32(s,0);if(font==EmojiFont)return state.size;if(font.characterLookupTable.TryGetValue(ch,out var glyph))return glyph.glyph.metrics.horizontalAdvance*state.size/font.faceInfo.pointSize;return state.size;}
