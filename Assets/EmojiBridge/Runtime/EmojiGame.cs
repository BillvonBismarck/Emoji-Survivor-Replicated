...
public sealed class EmojiGame : MonoBehaviour {
 ...
 Application.targetFrameRate=60;saveRoot=Path.Combine(Application.persistentDataPath,"EmojiSurvivorLua");Directory.CreateDirectory(saveRoot);
 doc=gameObject.AddComponent<UIDocument>();var settings=ScriptableObject.CreateInstance<PanelSettings>();settings.scaleMode=PanelScaleMode.ConstantPixelSize;settings.themeStyleSheet=Resources.Load<ThemeStyleSheet>("EmojiTheme");doc.panelSettings=settings;
 ...
 surface=new VectorSurface();surface.EmojiSequences=JsonConvert.DeserializeObject<Dictionary<string,string>>(Resources.Load<UnityEngine.TextAsset>("emoji-sequences").text);surface.MainFont=FontAsset.CreateFontAsset(Resources.Load<Font>("Fonts/MiSans-Regular"));surface.PixelFont=FontAsset.CreateFontAsset(Resources.Load<Font>("Fonts/zpix"));
 surface.EmojiFont=FontAsset.CreateFontAsset(Resources.Load<Font>("Fonts/EmojiUnity"),96,2,UnityEngine.TextCore.LowLevel.GlyphRenderMode.COLOR,2048,2048,AtlasPopulationMode.Dynamic,true);
 surface.MainFont.atlasPopulationMode=AtlasPopulationMode.Dynamic;surface.PixelFont.atlasPopulationMode=AtlasPopulationMode.Dynamic;surface.MainFont.fallbackFontAssetTable=new List<FontAsset>{surface.EmojiFont};surface.PixelFont.fallbackFontAssetTable=new List<FontAsset>{surface.EmojiFont,surface.MainFont};root.Add(surface);
 ...
 void Update(){if(Lua==null||surface==null||LastError!="")return;try{
 float scale=Mathf.Min(Screen.width/720f,Screen.height/1280f);surface.style.scale=new Scale(new Vector3(scale,scale,1));
