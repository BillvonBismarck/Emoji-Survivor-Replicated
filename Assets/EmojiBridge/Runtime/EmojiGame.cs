...
public sealed class EmojiGame : MonoBehaviour {
 ...
 Lua.Bind("unity_json_lua",()=>{try{return Lua.Push(JsonToLua(JToken.Parse(Lua.Str(1))));}catch(JsonException){return Lua.Push("");}});
 Lua.Bind("unity_exists",()=>Lua.Push(File.Exists(SavePath(Lua.Str(1)))));
 Lua.Bind("unity_read",()=>{try{return Lua.Push(File.ReadAllText(SavePath(Lua.Str(1))));}catch(IOException){return Lua.Push("");}catch(UnauthorizedAccessException){return Lua.Push("");}});
 Lua.Bind("unity_write",()=>{try{var p=SavePath(Lua.Str(1));File.WriteAllText(p+".tmp",Lua.Str(2));if(File.Exists(p))File.Replace(p+".tmp",p,null);else File.Move(p+".tmp",p);return Lua.Push(true);}catch(IOException){return Lua.Push(false);}catch(UnauthorizedAccessException){return Lua.Push(false);}});
 Lua.Bind("unity_master_audio",()=>{AudioListener.volume=Mathf.Clamp01((float)Lua.Num(1));return 0;});
 ...
 void OnDestroy(){AudioListener.volume=1;if(Keyboard.current!=null)Keyboard.current.onTextInput-=OnText;surface?.RemoveFromHierarchy();Lua?.Dispose();Lua=null;if(Instance==this)Instance=null;}
}
}
