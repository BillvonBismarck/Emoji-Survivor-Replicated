...
local excluded = {pool=true,spatialHash=true,charDef=true,bossDef=true,currentBossDef=true}
...
function Snapshot.Capture(metadata)
 ...
 weekly=W.active,weeklyRule=W.todayRule,weeklySeason=W.seasonId,weeklyDay=W.seasonDay}
...
function Snapshot.Restore(root)
 ...
 local C=require('Config');local P=require('battle.Player');P.charDef=C.GetCharacter(P.charId)
