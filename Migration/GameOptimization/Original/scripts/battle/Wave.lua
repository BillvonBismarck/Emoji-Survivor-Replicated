...
function Wave.Update(dt, playerX, playerY)
    ...
    if Wave.timer >= Config.WAVE.interval then
        ...
        local requested = GetWaveEnemyCount()
        -- Expected swarm pack pressure; one slot is always reserved for a boss.
        local swarmWeight = Wave.waveNum >= 13 and .09 or (Wave.waveNum >= 5 and .13 or 0)
        local virtualCount=requested*(1+3*swarmWeight)
        Wave.quantityFactor=math.min(8,math.max(1,virtualCount/(Config.WAVE.maxEnemiesAlive-1)))
        Wave.requestedCount=requested
        -- Spawn elites first so crowd saturation cannot silently suppress them.
        local eliteCount = DailyChallenge.active and 2 or 1
        for _=1,eliteCount do Enemy.SpawnAroundPlayer(playerX,playerY,'elite',Wave.waveCoeff) end
        local count=math.min(requested,math.max(0,Config.WAVE.maxEnemiesAlive-1-#Enemy.active))
        for _ = 1, count do
            if #Enemy.active >= Config.WAVE.maxEnemiesAlive-1 then break end
            local etype = PickEnemyType()
            if etype == "swarm" then
        ...
        -- 每波刷精英（每日挑战翻倍为 2 只）
        -- Elites were budgeted before the ordinary wave.
